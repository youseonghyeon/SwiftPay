...
이 문서에서는 120명의 가상 유저(VUs)가 30초 동안 API 엔드포인트에 요청을 보낸 성능 테스트 결과입니다.
...
이 성능 테스트 결과는 서버가 120명의 동시 사용자에 대해 30초 동안 안정적으로 동작하며, 모든 요청을 성공적으로 처리할 수 있음을 보여줍니다. 평균 응답 시간은 1초 이내로 나타났지만, 일부 요청에서 응답 시간이 2초를 넘는 경우가 있었으므로, 향후 최적화를 고려할 수 있습니다.

이 결과를 바탕으로, 서버의 성능이 사용자 요구 사항을 충분히 충족시키고 있음을 확인할 수 있습니다. 하지만 더 높은 트래픽이나 장기적인 부하 상황에서의 성능을 평가하기 위해 추가적인 테스트가 필요할 수 있습니다.

---

# 성능 테스트 결과 - 700명 30초 동안의 단순 API 요청

이 문서에서는 700명의 가상 유저(VUs)가 30초 동안 API 엔드포인트에 요청을 보낸 성능 테스트 결과를 기록합니다. 이번 테스트는 높은 부하 상황에서 서버의 성능과 안정성을 평가하기 위해 수행되었습니다.
...
이번 성능 테스트 결과는 서버가 700명의 동시 사용자에 대해 30초 동안 안정적으로 동작하며, 모든 요청을 성공적으로 처리할 수 있음을 보여줍니다. 그러나 평균 응답 시간이 3초 이상으로, 고부하 상황에서의 성능 개선이 필요할 수 있습니다.

향후 성능을 더욱 최적화하고, 높은 트래픽을 처리할 수 있도록 추가적인 개선 작업이 필요할 것으로 보입니다.

또한 이보다 많은 connection 을 만들려 하면 connection reset by peer 에러가 발생할 수 있으며, 이는 서버 설정을 통해 수정해야 합니다.

## 추가 진행 계획

서비스의 요구사항을 추가하여 리소스를 더 많이 사용하도록 기능을 확장할 예정입니다.
